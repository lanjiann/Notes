...
- [[1. Introduction - 1][1. Introduction - 1]]
  - [[Why Did They Need to Change Java Again? - 1 - =Re-Read=][Why Did They Need to Change Java Again? - 1 - =Re-Read=]]
...
- [[2. Lambda Expressions - 5][2. Lambda Expressions - 5]]
  ...
  - [[Using Values - 8 - *IMPORTANT*][Using Values - 8 - *IMPORTANT*]]
...
- [[3. Streams - 17 - *START*][3. Streams - 17 - *START*]]
...
- [[5. Advanced Collections and Collectors - 59][5. Advanced Collections and Collectors - 59]]
  ...
  - [[Enter the Collector - 62][Enter the Collector - 62]]
    ...
    - [[Composing Collectors - 67 - =Re-Read=][Composing Collectors - 67 - =Re-Read=]]
...
- [[8. Design and Architectural Principles -109][8. Design and Architectural Principles -109]]
  - [[Lambda-Enabled Design Patterns - 110][Lambda-Enabled Design Patterns - 110]]
    ...
    - [[Template Method Pattern - 119 - =Re-Read=][Template Method Pattern - 119 - =Re-Read=]]
  ...
  - [[Further Reading - 137 - =Read=][Further Reading - 137 - =Read=]]
...
** DONE Method References - 59
   ...
   - The standard form is ~Classname::methodName~
     ...
     You're providing the _equivalent_ of a /lambda expression/ that can be called
     in order to call the /method/.

   - Call constructors in the form of ~Classname::new~
...
** DONE Element Ordering - 60
   CLOSED: [2018-08-05 Sun 20:59]
   How elements are ordered in /streams/?

   - The situation with ordering becomes a little more complex with ~Stream~
     operations.

   - A ~Stream~ intuitively presents an order because each element is operated
     upon, or encountered, in turn.

     + We call this the /encounter order/.

     + How the /encounter order/ is defined _DEPENDS ON_ both
       * the *source* of the data

       * the *operations* performed on the ~Stream~.

   - When you create a ~Stream~ from a collection with a defined order, the ~Stream~
     has a defined /encounter order/.

   - If a ~Stream~ is created from data structures like ~HashSet~, there won't be
     /encounter order/.

     However, operations like ~.sorted()~ can create /encounter order/.

   - The /encounter order/ is propagated across itermediate operations if it exists.
   ...
   - Some operations are _more EXPENSIVE_ on /ordered streams/.
     ...
     Most operations, however, such as ~filter~, ~map~, and ~reduce~, can operate
     VERY EFFICIENTLY on /ordered streams/.

   - This can cause _unexpected behavior_,
...
*** DONE Into Other Collections - 62
    ...
    - =TODO= *parallel operations*
      ...
      concrete implementation of the List or Set . Under the hood, the streams
      library is picking an appropriate implementation for you. Later in this
      book I’ll talk about how you can use the streams library to perform data
      parallel operations; collecting the results of parallel operations can
      require a different type of Set to be produced than if there were no
      requirement for thread safety.

    - Convert streams to specific collections.
      For example, ~stream.collect(toCollection(TreeSet::new));~

*** DONE To Values - 63
    CLOSED: [2018-08-05 Sun 23:30]
    - It's also possible to _collect into a SINGLE value_ using a /collector/.
...
** TODO Collection Niceties - 77
   - ~computeIfAbsent~
     The example use a ~Map~ (~artistCache~) to cache expensive operations result.
