...
*** TODO 1.2 Boxing and Unboxing - 6
...
*** TODO 1.4 Generic Methods and Varargs - 10
    - Syntax:
      ~T... variable~ in parameter list.
...
** TODO 2. Subtyping and Wildcards - 15
   We'll review how /subtyping/ works;
   we'll see how /wildcards/ let you use /subtyping/ *in connection with* /generics/;
   We'll illustrate our points with examples from the _Collections Framework_.

*** DONE 2.1 Subtyping and the Substitution Principle - 15
    CLOSED: [2020-01-05 Sun 17:07]
    In Java,
    ...
    - Substitution Principle :: a variable of a given type may be assigned a value
         of any subtype of that type, and a method with a parameter of a given
         type may be invoked with an argument of any subtype of that type.
    ...
    - The /substitution principle/ does *NOT* regulate the /subtyping relation/
      ...
      + /Java array/ is not a part of the _Collection Framework_, and it follow a
        DIFFERENT (=from Jian= WRONG!!!) rule. (TODO: See Section 2.5)
...
*** DONE 2.2 Wildcards with ~extends~ - 17 - TODO RE-READ
    CLOSED: [2020-01-05 Sun 18:56]
    Example:
    #+begin_src java
    ...
    - One commmon error:
      ...
      2. In general, if a structure contains elements with a type of the form ~?
         extends E~, we
         + CAN get elements out of the structure,

         + BUT we CANNOT put elements into the structure.

      3. To put elements into the structure we need another kind of /wildcard/, as
...
*** TODO 2.4 The Get and Put Principle - 19
    - The Get and Put Principle :: use an extends wildcard when you only get values
         out of a structure, use a super wildcard when you only put values into
...
*** 3.1 Comparable - 31
    ...
    - When a class implements ~Comparable~, the ordering specified by this /interface/
      is called the /natural ordering/ for that class.
