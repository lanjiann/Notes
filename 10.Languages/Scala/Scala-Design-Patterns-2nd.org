...
- [[2. Traits and Mixin Compositions][2. Traits and Mixin Compositions]]
  ...
  - [[Linearization][Linearization]]
    ...
    - [[Linearization rules - =TODO= =NOTE=][Linearization rules - =TODO= =NOTE=]]
    - [[How linearization works - =TODO= =NOTE=][How linearization works - =TODO= =NOTE=]]
...
** DONE What are creational design patterns?
   CLOSED: [2019-05-10 Fri 14:20]
   - Motivation:
     _Creating objects_ could involve some extra complexities, which *make the
     use of software components harder*.
     _We want to *HIDE these complexities*._
...
** TODO Other factory design patterns
*** The static factory
*** The simple factory
...
*** An example class diagram
    - The Java way:
      ...
      + Declare the /fields/ as ~final~, and manually implement the ~hashCode~
        and ~equals~ methods.

*** A code example
*** DONE Alternative implementation
    CLOSED: [2018-12-14 Fri 15:55]
    - Tuples can be an alternative if the number of fields are less than 22 --
      the limitation of built-in tuple classes (will be changed in Scala 3).

    - Replace /case classes/ with /tuples/ can affect the readability!
      =from Jian= Use /2-tuple/ or /3-tuple/ in localized code snippets is OK.

    - However, accesss the fields of a /case class/ is much elegant than accesss
      the element of a tuple like ~tuple._3~.

*** DONE What it is good for
    CLOSED: [2018-12-14 Fri 16:05]
    - The value object design pattern is good for multithreading and creating
      /data transfer objects (DTOs)/.

*** DONE What it is not so good for
    CLOSED: [2018-12-14 Fri 16:05]
    As we mentioned, to implement the /value object design pattern/, you can
    use /case classes/ or/and /tuples/.

    For most cases (=from Jian= NOT for ALL cases), use /tuples/ to prepresent
    /value objects/ in Scala is the ONLY potential drawback to using this
    pattern.
