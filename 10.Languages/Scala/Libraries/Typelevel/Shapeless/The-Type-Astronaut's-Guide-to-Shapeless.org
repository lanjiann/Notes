...
** DONE 3.1 Recap: type classes - 21
   ...
   - Scala type class :: a /parameterised trait/ representing some sort of general
        functionality that we would like to apply to _a wide range of_ /types/.
   ...
   - Use the ~Employee~ type class instance of ~CsvEncoder~:
     ...
     For example, we can define a ~CsvEncoder[IceCream]~ instance, and then
     ~writeCsv[A]~ function can work with ~IceCream~.
