...
#+VERSION: 3.0.0-M2-bin-20201112-462a72f-NIGHTLY 
...
**** DONE Implementation
     ...
     - ~scala.reflect.Enum~ defines a _SINGLE_ /public method/ ~ordinal~:
       #+begin_src scala
         ...
         @mixin trait Enum extends Any with Product with Serializable {
...
**** DONE Pattern-Bound Given Instances
     CLOSED: [2020-11-09 Mon 11:54]
     /Given instances/ can also appear as /pattern bound-variables/.
...
*** DONE Mixin Traits
    ...
    - /Mixin traits/ is designed to address the topic of hiding /mixin traits/ from
      /inferred types/.
      * Example:
        #+begin_src scala
          trait Kind
    ...
    - Scala 3 allows one to mark a /trait/ as a ~@mixin~ /trait/, which means that
...
**** Mixin Traits
     - The /traits/ ~scala.Product~, ~java.lang.Serializable~ and ~java.lang.Comparable~
       are *treated _AUTOMATICALLY_ as /mixin traits/.*
       * Other /traits/ can be _TURNED INTO_ /mixin traits/,
         _BY ADDING_ the /annotation/ ~@mixin~ in front of ~trait~, as shown above.

     - Every /trait/ *can be* declared as a /mixin trait/.

     - Typically /mixin traits/ are /traits/ that influence the implementation of
     ...
     - =IMPORTANT=
       ...
       to be declared a /mixin trait/.

**** Rules for Inference - =TODO= =RE-READ= =NEED-FIX???=
     ...
     - The _last clause_ *ensures* that
       a single /mixin trait/ instance such as ~Product~ is not widened to
       ~Any~.
       + =IMPORTANT= =SUMMARY=
         /Mixin trait/ instances are _ONLY dropped_ when they appear _in conjunction
...
*** DONE ~targetName~ Annotation
    CLOSED: [2020-11-12 Thu 00:10]
    ...
    - The /implementation name/
      ...
      * is the _NAME_ under which code _from OTHER languages_ can call the /method/.
        For instance, ~++=~ could be invoked from Java like this:
        #+begin_src java
          VecOps.append(vec1, vec2)
        #+end_src
        =IMPORTANT= *ONLY from OTHER languages! You can't use the name in Scala.*

***** Details
      1. ~@targetName~ is defined in package ~scala.annotation~.
         It takes a _single argument_ of /type/ ~String~.
      ...
      2. An ~@targetName~ annotation can be given for *ALL* kinds of definitions.
         =from Jian= *NOT ONLY* for symbolic name method.

      3. The given /external name/ must be a legal name for the defined entities
         on the host platform.

      4. It is *RECOMMENDED* that definitions with /symbolic names/ have a
         ~@targetName~ annotation. This will establish an _alternate name_ that
         * is EASIER TO *search for*
           and
         * will avoid cryptic encodings in runtime diagnostics.

      5. Definitions with _names in backticks_ that are *NOT legal* _host platform names_
         *should* also have a ~@targetName~ annotation.

      6. =REMOVED= =TODO=
         =from Jian= WHY this is removed??? =???= =TODO=
         ~@targetName~ annotations must agree:
         *There must be a one-to-one relationship between external and internal
         names*
         
***** Relationship with Overriding - =RE-READ= =TODO=
      - ~@targetName~ annotations are significant for matching _TWO_ /method
        ...
        TWO /method dfinitions/ *match* IF they have the SAME _name_, /signature/,
        and /erased name/. Here,
        * The /signature/ of a definition consists of
          + the _names_ of
            _the /erased types/ of all (value-) parameters_ and

          + the /method/'s /result type/.

        * The _erased name_ of a /method definition/ is
          its /target name/ IF a ~@targetName~ annotation is given
          and 
          its _defined name_ OTHERWISE.

      - This means that ~@targetName~ annotations can be used to *disambiguate*
        TWO /method definitions/ that would otherwise clash. For instance.
        #+begin_src scala
        ...
        both ~Function0~, which is the /type/ of the translation of a /by-name-parameter/.
          Hence they have the _SAME names and signatures_. But we can avoid the
      ...
      - However, ~@targetName~ annotations are *NOT allowed to BREAK* /overriding
        ...
        and types_. So the following would be in error:
        #+begin_src scala
      ...
      - The relevant _overriding rules_ can be summarized as follows:
        * Two members can *override* each other if their names and /signatures/ are
          the same, and they either have the same /erased names/ or the same /types/.
      ...
      - As usual, any /overriding relationship/ in the _generated code_ *must also be
        ...
        #+end_src
        Here, the ORIGINAL methods ~g~ and ~f~ _do NOT *override* each other_ since
        they have _DIFFERENT names_. BUT once we switch to /target names/, there
        is a clash that is reported by the compiler:
        #+begin_src text
...
**** TODO (Deprecated, Updated by spliting this to two parts) Principles and Rules
...
*** DONE Operators
    ...
    *Rules for Operators*
    - The rules for infix operators have changed in some parts:
      1. an alphanumeric method can be used as an infix operator only if its
         definition carries an @infix annotation.
...
**** DONE The ~@targetName~ Annotation
     - It is *RECOMMENDED* that
       ...
       is a "~@targetName~ annotation" in this document) that provides an encoding
       of the _operator with an ALPHANUMERIC NAME._
     ...
     - =from Jian=
       The above list mentioned multiple times of *low-level encoding*.
       _ALPHANUMERIC NAMEs_ are much more widely accepted by other languages,
       and tools.
