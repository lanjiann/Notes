...
#+VERSION: 3.0.0-RC1-bin-20201125-1c3538a-NIGHTLY
...
*** DONE Inline - =TODO= =FIX= apply the universal access rule and eliminate useless empty parentheses
...
**** DONE From ~Expr~'s to Functions and Back
     ...
     - Example:
       =from Jian= =TODO= Need to create a /given instance/ of ~Quotes~
       #+begin_src scala
         // '{ (x: Int) => x.toString }
     ...
     - The ~Expr.apply~ method is defined in package ~quoted~:
       #+begin_src scala
         ...
         object Expr {
           ...
           def apply[T: Liftable](x: T)(using Quotes): Expr[T] =
       ...
       #+begin_src scala
         object Macros {
           def map[T](arr: Expr[Array[T]], f: Expr[T] => Expr[Unit])
                     (using t: Type[T], quotes: Quotes): Expr[Unit] = '{
             var i: Int = 0
             while (i < ($arr).length) {
               val element: $t = ($arr)(i)
               ${f('element)}
...
**** (C) ~Expr~ with ~@compileTimeOnly("Illegal reference to `scala.quoted.runtime.Expr`")~
