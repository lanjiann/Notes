...
#+VERSION: 0.26.0-bin-20200716-03aa171-NIGHTLY
...
**** DONE Translation of Enumerations
...
*** DONE Given Imports
    ...
    - Example:
      ...
      #+end_src
      + In Dotty,
        * ~import A._~ import all members of ~A~ *EXCEPT* the /given instances/
          ~tc~ -- this is different from Scala 2.
        * Conversely, ~import A.{given _}~ will import *ONLY* that /given instance/.

      + Merge these two import clauses into one ~import A.{given _, _}~.
        This can import everything in ~A~.

    - There are _TWO main benefits_ arising from these rules:
      ...
      + It ENABLES importing ALL /givens/ WITHOUT importing anything else.
        ...
        =from Jian= next section will introduce /importing (/givens/) by type/.

**** DONE Importing By Type
     CLOSED: [2020-07-12 Sun 00:52]
     ...
     - Examples:
       ...
       + Example code:
         ...
         instances but leave out the ~im~ instance, since it fits non of the
         specified bounds.
...
**** DONE Migration
     CLOSED: [2020-07-12 Sun 01:05]
     - The rules for imports stated above have the consequence that a library
       would have to migrate in lockstep with all its users from old style
       implicits and normal imports to givens and given imports.
     ...
     - These rules mean that library users
       + can use /given _ selectors/ to ACCESS /old-style implicits/ in Scala 3.0,
         AND
...
**** DONE Generic Extensions
     ...
     - When calling a /generic extension method/, any EXPLICITLY given /type
       arguments/ follow the /method/ NAME (of course, usually they are left out
     ...
     - Note:
       ...
       + _This RESTRICTION might be lifted_
         *in the future ONCE* we support /multiple type parameter clauses/ in a
         /method/.
           By contrast, there can be /using clauses/ in front as well as after
         the ~def~.

**** DONE Collective Extensions - =TODO= =!!!=
     CLOSED: [2020-07-12 Sun 02:07]
     ...
     and enclose all methods in the following braces or indented region followed by ~:~.
     =from Jian= The exmaple code doesn't include this mentioned ~:~!!!
     =TODO= CREATE a github Issue to ask!!! =TODO=
     Examples:
     #+begin_src scala
     ...
     + /Collective extensions/ also can take /type parameters/ and have /using clauses/.
       Example:
       #+begin_src scala
...
**** DONE Syntax
     CLOSED: [2020-07-12 Sun 02:20]
     - ~extension~ is a /soft keyword/.
       + It is recognized as a keyword only if it appears at the start of a
         statement and is followed by ~[~ or ~(~.
...
*** DONE Implementing Type classes
    ...
    - Examples of use-cases:
      ...
      + expressing such a behavior for multiple types without involving /sub-typing/
        relationships (one extends another) between those types (see: /ad hoc
...
***** ~Reader~
      ...
      - We may want to combine compute and show into a single function, accepting
        a Config as parameter, and showing the result of the computation, and
        we'd like to use a monad to avoid passing the parameter explicitly
        multiple times. So postulating the right flatMap operation, we could
        write: ~def computeAndShow(i: Int): Config => Unit = compute(i).flatMap(show)~
        instead of ~show(compute(i)(config))(config)~

      - Let's define this m then.
        First, we are going to define a type named ConfigDependent representing
        a function that when passed a Config produces a Result.
...
**** DONE Summary
     ...
     - To conclude, we have seen that
       ...
       + /context bounds/
       + /type lambdas/
       allow a concise and natural expression of /type classes/.
