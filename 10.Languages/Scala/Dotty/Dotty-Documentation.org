...
#+VERSION: 3.0.0-M2-bin-20201107-5c84e42-NIGHTLY
...
**** DONE Translation of Extension Methods
     CLOSED: [2020-11-08 Sun 13:44]
     - extension methods :: /methods/ that have a parameter clause in front of the
       defined identifier.

     - An /extension method/ translates to a /specially labelled method/ that
       TAKES the _leading parameter_ section AS its _first argument list_.
       + The label, expressed as <extension> HERE, is compiler-internal.

     - So, the definition of ~circumference~ above translates to the _PLAIN_
...
**** DONE Example: Builder Pattern
     CLOSED: [2020-11-08 Sun 14:40]
     ...
     - Then, the ~table~, ~row~ and ~cell~ /constructor methods/ can be defined
       ...
       #+begin_src scala
         def table(init: Table ?=> Unit) = {
           // note the use of a creator application; same as: given t as Table = new Table
           given t as Table
           init
           t
         }
...
**** DONE Example: Postconditions
     CLOSED: [2020-11-08 Sun 14:41]
     ...
     - Explanations:
       ...
       + Hence, the implementation of ~ensuring~ is as about _as *efficient* as the
         ...
         * =from Jian= =TODO= =Verify=
           ...
           - My guess of the expansion:
             #+begin_src scala
               val s = List(1, 2, 3).sum.ensuring(result == 6)

               val s = List(1, 2, 3).sum.ensuring(WrappedResult[Int] ?=> result == 6)

               // Maybe not legal syntax, just a illustration
               val s = List(1, 2, 3).sum
                                    .ensuring { (using r: WrappedResult[Int]) =>
                                      result == 6
                                    }
...
*** DONE By-Name Context Parameters - =TODO= =RE-READ=
    ...
    /Context parameters/ can be DECLARED /by-name/ to *avoid* a /divergent
    inferred expansion/.
    ...
    - The precise steps for /synthesizing an argument/ for a /by-name context
      ...
      2. This /given/ is NOT immediately available as candidate for /argument
         ...
         the _synthesized computation_). But it becomes available in all nested
