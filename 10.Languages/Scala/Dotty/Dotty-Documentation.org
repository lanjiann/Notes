...
#+VERSION: 3.0.0-RC1-bin-20201128-928e01a-NIGHTLY
...
***** DONE The ~scala.compiletime.ops~ package
      ...
      *primitive operations on /singleton types/.*
      ...
      - Many of these /singleton operation types/ are meant to be used _infix_
        (as in SLS § 3.2.8), and are annotated accordingly woth ~infix~ /modifiers/.
...
**** DONE From ~Expr~'s to Functions and Back - =RE-READ=
...
**** DONE Lifting Types - =READING= =START=
     ...
     - */Types/ are _NOT_ directly affected by the /phase consistency principle/.*
      ...
      - *To _AVOID_ clutter* (=MOTIVATION= of /lifting types/),
       ...
       + For instance, the user-level definition of ~to~:
         #+begin_src scala
           def to[T, R](f: Expr[T] => Expr[R])
                       (using t: Type[T], r: Type[R], Quotes): Expr[T => R] =
             '{ (x: T) => ${ f('x) } }
         #+end_src
