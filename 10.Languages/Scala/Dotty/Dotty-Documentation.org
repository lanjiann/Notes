...
#+VERSION: 0.25.0-bin-20200603-cc8d6c3-NIGHTLY
#+AUTHOR:
...
**** TODO The New Design
     ...
     - This section also contains pages describing other language features that
       ...
       + /Extension Methods/ REPLACE /implicit classes/ in a way that _INTEGRATES
         BETTER with /typeclasses/._

       + /Implementing Typeclasses/ demonstrates how some common /typeclasses/ can
         be implemented using the new constructs. TODO ??? TODO

       + /Typeclass Derivation/ introduces constructs to AUTOMATICALLY *derive*
         /typeclass instances/ for ADTs.
...
*** DONE Multiversal Equality - TODO =RE-READ=
    ...
    - /Multiversal equality/ is an _opt-in way_ to make /universal equality/ SAFER.
        It uses a /binary typeclass/ ~Eql~ to indicate that values of *two* given
    ...
    - If we want to disable /universal equality/ check for ~T~, we can do
      #+begin_src scala
        ...
        x == y       // can't typecheck because T drevies the typeclass Eql
      #+end_src
...
**** DONE Deriving ~Eql~ Instances
     ...
     - By the usual rules of /typeclass derivation/, this generates the following
...
*** DONE Main Functions
    ...
    - Parameters of the ~@main~ method:
      ...
      + For each parameter type there *must be* an instance of the
        ~scala.util.FromString~ /typeclass/ that is used to
