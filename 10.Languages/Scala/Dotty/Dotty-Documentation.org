...
#+VERSION: 3.0.0-M2-bin-20201111-306e4fc-NIGHTLY
...
*** TODO ~targetName~ Annotation
    ...
    - An ~@targetName~ /annotation/ will be *MANDATORY*
      _if the /method name/ is symbolic_!!!
      + Symbolic name methods without ~@targetName~ annotation are *DEPRECATED*.

***** Details
      1. Syntax:
         ~@scala.annotation.targetName(externalName)~.
      ...
      4. Lack of an ~@targetName~ annotation will raise a _deprecation warning_.

      5. Definitions with names in backticks that are not legal host platform
         names should have an ~@targetName~ annotation. Lack of such an annotation
         will raise a deprecation warning.
         =from Jian= TODO EXAMPLE???

      6. ~@targetName~ annotations must agree:
         *There must be a one-to-one relationship between external and internal
         names*
         
***** Relationship with Overriding
      - ~@targetName~ annotations are significant for matching two method definitions
        to decide whether they conflict or override each other. Two method definitions
        match if they have the same name, signature, and erased name. Here,
        * The signature of a definition consists of the names of the erased types
          of all (value-) parameters and the method's result type.
      ...
      - This means that @targetName annotations can be used to disambiguate two
        method definitions that would otherwise clash. For instance.
        #+begin_src scala
      ...
      - The two definitions above clash since their erased parameter types are
        both Function0, which is the type of the translation of a
        by-name-parameter. Hence they have the same names and signatures. But we
        can avoid the clash by adding a @targetName annotation to either method
        or to both of them. E.g.
