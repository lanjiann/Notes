...
#+VERSION: 0.20.0-bin-SNAPSHOT -> ... -> 0.23.0-bin-20200213-26de50a-NIGHTLY
#+AUTHOR:
...
**** DONE Unsoundness - =IMPORTANT=
     CLOSED: [2019-12-31 Tue 02:32]
     ...
     - The /unsoundness/ happens because uninitialized fields in a class start out
       as ~null~:
       + =from Jian= The code in the document doesn't compile
         #+begin_src scala
           class C {
             val f: String = foo(f)
           ...
           // c.f == "field is null"
         #+end_src
         Because of the code ~f2 == null~ is not legal when compile with
         ~-Yexplicit-nulls~. Therefore, this code can't show the unsoundness --
         must pass the compilation but break some contract(s).

       + =from Jian= Corrected and simplified version:
         #+begin_src scala
           // -Yexplicit-nulls
           class C with
             val f: String = foo(f)
