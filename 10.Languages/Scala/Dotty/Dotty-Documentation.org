...
#+VERSION: 3.0.0-RC1-bin-20201224-2a32d6a-NIGHTLY
     ...
     - For any query /type/ ~Q~, ~NotGiven[Q]~ *succeeds*
       ...
       #+begin_src scala
         ...
         @main def test() =
           ...
           assert(sommon[Foo[Int]].value)      // fooTagged is found
           assert(!sommon[Foo[String]].value)  // fooNotTagged is found
       #+end_src
       =FIXME= =IMRPOVE ME=
     ...
     *ALL* data types CAN HAVE _a ~derives~ clause_.
     ...
     - Note the following properties of ~Mirror~ /types/,
       ...
       + The /kinds/ of ~MirroredType~ and ~MirroredElemTypes~ match the /kind/
         ...
         * This allows ~Mirror~'s to support /ADTs/ of *all* /kinds/. =FIXME=
...
**** DONE Inline Matches
     ...
     - The /type/ can have a RICHER structure like the _simple_ /ADT/ below.
       ...
       =IMPORTANT= =GOOD EXAMPLE=
       =IMPORTANT= =GOOD EXAMPLE=
       =FIXME= =???= =Replace the final with inline= =I remember use final to declare constants will be phased out=
...
*** TODO TASTy Inspection**** Inspecting TASTy files
**** Template project
...
*** DONE Universal Apply Methods
    ...
    - Example:
      ...
      #+end_src
      This works since a /companion object/ with TWO ~apply~ /methods/ are =FIXME=
    ...
    - /Constructor proxies/ are generated _even for_ /Java classes/ and /classes/
      ...
      * The precise rules are as follows:
        1. A /constructor proxy/ /companion object/ =FIXME= ~C~ is created for a
...
**** More Details
     - Spec diff:
       in section _5.1.4 Overriding_, =FIXME= - =link to 2.13 instead of 2.12=
