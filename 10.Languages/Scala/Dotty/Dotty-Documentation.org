...
#+VERSION: 3.0.0-RC1-bin-20201130-b2a09d3-NIGHTLY
...
**** TODO Pattern matching on quoted expressions
     ...
     - ~scala.quoted~ contains objects that can help extracting values from ~Expr~: 
       * ~scala.quoted.Const~ / ~scala.quoted.Consts~:
         matches an expression of a literal value (or list of values) and
       ...
       * ~scala.quoted.Varargs~:
         matches an explicit sequence of expresions and returns them. These
...
***** TODO Quoted patterns
      ...
      structure, types or methods. Patterns ~'{ ... }~ can be placed in any
      location where Scala expects a pattern.
      For example,
      #+begin_src scala
        optimize {
          sum(sum(1, a, 2), 3, b)
