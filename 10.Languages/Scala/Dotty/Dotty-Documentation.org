...
#+VERSION: 0.27.0-bin-20200731-bb23fea-NIGHTLY
...
*** TODO Numeric Literals
    ...
    - Scala 3 allows to write /numeric literals/ also for /user defined types/.
      ...
      *except* there are *NO* _pre-set limits how large they can be_.

**** TODO Meaning of Numeric Literals
     ...
     - With these rules, the definition =FIX-DOC=
       ~val x: Long = -10_000_000_000~
       is legal by rule 1., since the _expected type_ is ~Long~. The definitions
       #+begin_src scala
         val y: BigInt = 0x123_abc_789_def_345_678_901
       ...
       and ~FromDigits.Decimal~, respectively). On the other hand,
       ~val x = -10_000_000_000~
       gives a type error, since without an _expected type_ ~-10_000_000_000~ is
       treated by rule (3) as an ~Int~ literal, but it is too large for that type.

**** TODO The ~FromDigits~ Trait
     ...
     - ~FromDigits~ is defined as follows:
       #+begin_src scala
         trait FromDigits[T] {
     ...
     - Implementations of the ~fromDigits~
       ...
       + Number separator characters _ are filtered out before the string is
         passed to ~fromDigits~.

     - The /companion object/ ~FromDigits~ also defines /subclasses/ of ~FromDigits~
       for /whole numbers/ with a given _radix_, for _numbers with a decimal point_,
       and for _numbers that can have both a decimal point and an exponent_: =FIX-DOC=
       #+begin_src scala
         object FromDigits {
...
**** TODO Error Handling
     ...
     /subtype/ of ~FromDigitsException~. ~FromDigitsException~ is defined with _THREE_
     /subclasses/ in the ~FromDigits~ /object/ as follows:
     #+begin_src scala
