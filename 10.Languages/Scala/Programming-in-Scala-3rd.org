...
** DONE 11.1 Scala's class hierarchy - 246
   ...
   - The ~==~ /method/ is essentially the same as ~equals~
     and
     the ~!=~ /method/ is _ALWAYS_ the negation of ~equals~.

     footnote 1:
     + The only case where ~==~ does _NOT directly call_ ~equals~ is for Java's
       /boxed numeric classes/, such as ~Integer~ or ~java.lang.Long~ (must prefix
       ~java.lang~, or ~Long~ is the Scala built-in, not the Java one).
       * In Java, a ~new Integer(1)~ does NOT equal a ~new java.lang.Long(1)~ even
         though for /primitive values/ ~1 == 1L~ is true.

     + Since _Scala is a *more regular* language than Java,_ it was necessary to
       correct this discrepancy by special-casing the ~==~ method for these
       classes. Likewise, the ~##~ method provides a Scala version of hashing
       that is the same as Java's ~hashCode~, _EXCEPT for /boxed numeric types/,_
       where it works consistently with ~==~. For instance ~new Integer(1)~ and
       ~new Long(1)~ hash the same with ~##~ even though their Java ~hashCodes~
       are different.
       =IMPORTANT= =!!!=

   - =from Jian=
     As a summary of the note above,
     in Scala, if you are _NOT implementing_ the /methods/ originate from ~Any~,
     you should keep using ~==~ and ~##~, rather than ~equals~ and ~hashCode~.

     + You can get the hash of ~null~ by calling ~##~,
       but if you use ~hashCode~, a ~NullPointerException~ will be thrown!
   ...
   - ~Any~ has two direct subclasses:
     ...
     + The instances of /these classes/ are all written as /literals/ in Scala.
       You _CANNOT_ create instances of these classes using ~new~.
         This is enforced by the "trick" that /value classes/ are all defined to
       be both ~abstract~ and ~final~.

     + =from Jian=
       Currently, JVM only support /single-value value class/ (Java, till Java
       12, does NOT support /value class/).
       _JVM will support /mult-value value class/ in the future._
       Java will support /value class/ (single/multiple-value) in the future.

     + ~AnyRef~ :: the /base class/ of all /reference classes/ in Scala.

       * This is just an _alias_ for class ~java.lang.Object~. So /classes/ written
         in Java, as well as /reference classes/ written in Scala, all inherit
         from ~AnyRef~.
           One way to think of ~java.lang.Object~, therefore, is as the way
         ~AnyRef~ is implemented on the Java platform.

       * Thus, although you CAN use ~Object~ and ~AnyRef~ INTERCHANGEABLY in
         Scala programs on the Java platform,
...
** DONE 11.2 How primitives are implemented - 250
   ...
   - For /reference types/ _other than_ Java's /boxed numeric types/, ~==~ is treated
   ...
   - Class ~AnyRef~ defines an additional ~eq~ /method/, which _CANNOT be overridden_
     and is implemented as /reference equality/ (behaves like ~==~ in Java for
     /reference types/). It has a negation ~ne~.
     #+BEGIN_SRC scala
...
** DONE 11.4 Defining your own value classes - 253 - TODO
   CLOSED: [2019-06-09 Sun 03:39]
   ...
   - To define a /value class/, make it a /subclass/ of ~AnyVal~ , and put ~val~
     before the _one parameter_. Here is an EXAMPLE /value class/:
     #+BEGIN_SRC scala
...
*** DONE Avoiding a types monoculture - TODO
    CLOSED: [2017-10-12 Thu 00:15]
    =from Jian= The example here we use to illustrate this topic uses /value
    classes/. However, *avoiding a types monoculture* is a more general
    guideline.
    ...
    purposes. Even if such a /class/ is a so-called /tiny type/ with _NO
    /methods/ or /fields/,_
    *defining the additional /class/ is a way to help the compiler be helpful to
    you.*

    - Without /domain concept types/, even if you write in Scala, a _statically
      typed and strong typed language_, the compiler can't help -- often you use
      same types for diferent /domain concept/ if you don't try to define new
      /doamin concept types/.

    - Example:
      =TODO=
      The expected output is
      _<a id='chap:vcls'><h1 class='bold'>Value Classes</h1></a>_

      + without domain concept types:
        #+begin_src scala
          def title(text: String, anchor: String, style: String): String =
            s"<a id='$anchor'><h1 class='$style'>$text</h1></a>"

          // Wrong input, but not detected
          title("chap:vcls", "bold", "Value Classes")
          // res17: String = <a id='bold'><h1 class='Value Classes'>chap:vcls</h1></a>
        #+end_src

      + with domain concept types:
        #+begin_src scala
          sealed trait HtmlElement extends Any {
            def value: String
            override def toString = value
          }
          class Anchor(val value: String) extends HtmlElement
          class Style(val value: String)  extends HtmlElement
          class Text(val value: String)   extends HtmlElement
          class Html(val value: String)   extends HtmlElement
    ...
    - =from Jian=
      The code above is still not concise!
      See /opaque type/ in Scala 3.
      Similar things are supported in Go.

** DONE 11.5 Conclusion - 256
   CLOSED: [2019-06-09 Sun 03:02]
   In this chapter we showed you the /classes/ at the *top* and *bottom* of
   _Scala's class hierarchy_.
...
* DONE 13 Packages and Imports - 276
  ...
  - coupling :: the extent to which the various parts of the program rely on the
                other parts
    + *LOW* /coupling/ _reduces_ the risk that a SMALL, SEEMINGLY INNOCUOUS change
      in one part of the program will have DEVASTATING consequences in another
      part.
  ...
  - =from Jian=
    ...
    style/ is also possible -- though it may not be concise and clean.
...
** DONE 13.1 Putting code in packages - 277
   ...
   - Two ways to place code into named packages:
     + place the contents of an ENTIRE FILE into a /package/:
       use the ~package~ clause at the top of the file.
       #+BEGIN_SRC scala
...
** DONE 13.3 Imports - 285
   ...
   - Scala imports are actually _much more general_ than Java.
...
*** DONE Scope of protection
    CLOSED: [2019-05-27 Mon 02:18]
    =from Jian= Is this title misleading???
    ...
    - The same technique is *NOT possible* in Java.
      ...
      + =from Jian= However, comparing to the formal Java 9+ the built-in /module
                    systems/, what does Scala need to _complete_ a full /module
                    system/?? I remember Scala 3 has ~export~??
                    =TODO= =TODO= =TODO=
...
** DONE 13.6 Package objects - 291 
   ...
   - /Package objects/ are _compiled to_ /class files/ named ~package.class~ that
     are located in the directory of the /package/ that they augment.
...
** DONE 13.7 Conclusion - 293
   CLOSED: [2019-05-27 Mon 04:20]
   - Scala's system is the same in spirit as Java's /packages/,
     but there are some differences where Scala chooses to be more consistent or
     more general.

   - =TODO= =TODO= =TODO=
     Looking ahead,
     _Chapter 29_ describes a _MORE flexible /module system/ THAN_ division into
     /packages/. In addition to letting you separate code into several
     /namespaces/, that approach allows /modules/ to be *parameterized* and
     *inherit* from each other.

* DONE 14 Assertions and Tests - 294 - _No details in this book!!!_
  CLOSED: [2018-04-10 Tue 22:20]
  /Assertions/ and /tests/ are *two* important ways you can check that the
  software you write behaves as you expect.
...
* DONE 27 Annotations - 637 TODO *Learn MORE*
...
* DONE 29 Modular Programming Using Objects - 659 - TODO =Re-READ=
