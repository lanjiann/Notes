...
** =DEPRECATED= 1.10. Add Your Own Methods to the String Class - 25
*** DONE Problem - 25
...
** TODO 1.11. How to Create Your Own String Interpolator - ???
*** DONE Problem
...
** DONE 1.12. Create Random Strings - ???
...
** =DEPRECATED= 3.1. Looping with ~for~ and ~foreach~ - 54
...
*** Solution
*** Discussion
** =DEPRECATED=  3.4. Creating a for Comprehension (for/yield Combination) - 63
...
** DONE 4.7. Overriding Default Accessor and Mutators - 116 - =2020=
...
** =DEPRECATED= 4.7. Preventing Getter and Setter Methods from Being Generated - 119 - =2020=
*** Problem - 119
    - We know
...
*** Solution - 119
    - A :: Use /modifier/ ~private~ or ~private[this]~.
      + =from Jian=
        This book didn't mention the details!!!
        * ~private[this]~ turn off the /getter/ and /setter/ generation of the
          marked fields completely.

        * ~private~ make the generated /getter/ and /setter/ _private_, not stop
          generating!
    ...
    - Example:
      ...
      #+end_src
      + Disassemble it with =javap=, and you'll see this public API:
        #+begin_src java
        ...
        #+end_src
        * =from Jian=
          In the book there is code about the /superclass/:
          ~extends java.lang.Object implements scala.ScalaObject~.
            However, when use the newest version of Scala, the output of =javap=
          doesn't inlcude this part. I guess the reason is: Scala already removed
          ~scala.ScalaObject~ and only ~extends java.lang.Object~ is always true,
          and it is hidden =javap=.
...
**** Object-private fields - 120
     ...
     - =from Jian=
       Scala 3 will deprecate ~private[this]~ without any replacement. The
       reason is Scala 3 compiler can infer the ~[this]~ part, and there is no
       need to wirte it out manually.
       + When compiler infers that a field is _object private_ (as ~private[this]~
         in Scala 2), it won't generate a /getter/ and /setter/ for it.
         * The compiler will also not do /variance check/ for the _object private_
           /method/, though we don't care about this under the topic of this section.

** TODO 4.8. Preventing Accessor and Mutator Methods from Being Generated - 119
** DONE 4.9. Assigning a Field to a Block or Function - 121
...
** DONE 4.10. Setting Uninitialized var Field Types - 122 - =2020=
   CLOSED: [2020-04-30 Thu 12:12]
*** Problem - 122
*** Solution - 123
...
** DONE 4.11. Handling Constructor Parameters When Extending a Class - 124 - =2020=
...
** DONE 4.12. Calling a Superclass Constructor - 127 - =2020=
...
** DONE 4.13. Generating Boilerplate Code with Case Classes - 136 - =2020= - =RE-READ=
...
** =DEPRECATED= 4.14. Defining Properties in an Abstract Base Class (or Trait) - 131 - =2020=
   CLOSED: [2020-05-06 Wed 23:25]
*** Problem - 131
    - Q :: How to define _abstract or concrete properties_ in an /abstract base
           class (or trait)/ that *can be referenced in all /child classes/?*
...
**** Abstract ~val~ and ~var~ fields - 131
     ...
     - Concrete field implementations are presented in the Discussion, because it
       helps to understand _HOW the Scala compiler *translates* your code in the
       preceding examples_.

*** Discussion - 132
    - /Abstract fields/ related code generation:
      + /abstract ~var~ field/:
        /getter/ and /setter/
      ...
      + When you _define_ an /abstract field/ in an /abstract class/ or /trait/,
        the Scala compiler does *not create* a field in the resulting code.

    - ~-Xprint:all~

**** Concrete ~val~ fields in abstract classes - 133
     - Override the concrete ~val~ fields in /concrete subclass/:
       #+begin_src scala
         abstract class Animal {
           val greeting = { println("Animal"); "Hello" }  // provide an initial value
           def sayHello { println(greeting) }
           def run
         }

         class Dog extends Animal {
           override val greeting = { println("Dog"); "Woof" }  // override the value
...
** =DEPRECATED= 4.12. When to Use an Abstract Class - 129 - =2020=
   CLOSED: [2020-05-06 Wed 10:40]
*** Problem - 129
    - The reason why we have this question:
      ...
      + =from Jian=
        Scala 3 ~trait~ can have parameters. It seems there is no longer any reason
        to to use /abstract class/. *NOT SURE!!!* TODO
...
*** Solution - 129
    - There are _TWO main reasons_ to use an /abstract class/ in Scala:
      + You want to create a /base class/ that requires /constructor arguments/.
        =from Jian= This reason will disappear in Scala 3.

      + The code will be *called FROM Java code*.
        =from Jian= TODO Scala 2 ~trait~ powered by JDK8+ also eliminate this reason??? TODO
    ...
    - Regarding the SECOND reason, if you're writing code that needs to be
      ...
      TODO
      + =from Jian=
        Is this still an issue when use Scala 2.12+ and JDK 8+?

*** Discussion - 129
    - Use an /abstract class/ whenever a base behavior *must* have /constructor
      parameters/.
      + =from Jian=
        Here is a case that is NOT "MUST", thought the /trait/ version works,
        the /abstract class/ way is conciser!
        * If the a constructor parameter is ~val~ or ~var~:
          #+begin_src scala
            abstract class Animal(val name: String)
          #+end_src

          Can be defined in /trait/ as
          ...
          #+end_src
          - To extend the /abstract class/ version ~Animal~, we can do:
            #+begin_src scala
              class Dog(name: String) extends Animal(name)
            #+end_src

          - To extend the /trait/ version ~Animal~, we can do:
            #+begin_src scala
              class Dog(_name: String) extends Animal {
                val name = _name
              }
            #+end_src

        * If the a constructor parameter is passed in but not marked ~val~ or
          ~var~ (in this case, the construction through /trait/ is more error-prone;
          it need manually call the ~a_static_method~. The same job can be done
          with the /abstract class/ in its /constructor/):
          #+begin_src scala
            abstract class Animal(nm: String) {
              val name = a_static_method(nm)
            }
          #+end_src
          ...
          - To extend the /trait/ version ~Animal~, we can do:
            #+begin_src scala
              class Dog(_name: String) extends Animal {
                val name = a_static_method(_name)
              }
            #+end_src

          #+begin_src scala
            class Dog(nm: String) extends Animal(nm + "_X")
          #+end_src
    ...
    - Because a /class/ can extend *only one* /abstract class/,
      ...
      _ALWAYS_ use a /trait/, _UNLESS_ you have this /specific need/ (TODO ???
      TODO) to have /constructor arguments/ in your base implementation.

** DONE 4.15. Writing an ~unapply~ Method (Extractor) - ???
** DONE 4.16. Creating Inner Classes - 143 - =2020=
   CLOSED: [2020-05-08 Fri 01:12]
*** Problem - 143
    - Q :: You want to create a class as an /inner class/ to help _keep the class
           out of your public API_, or to otherwise encapsulate your code.

    - =from Jian=
      The most important part is not the _Solution_, which looks similar to Java
      but actually not! The _Discussion_ will explain!!!
...
*** Solution - 143
    ...
    #+begin_src scala
      ...
      class PandorasBox {
        ...
        val things = new mutable.ArrayBuffer[Thing]()
...
*** Discussion - 144
    ...
    - =from "The Java™ Tutorials"=
      Terminology:
      /Nested classes/ are divided into _TWO_ categories: *static* and *non-static*.
      + Nested classes that are declared ~static~ are called /static nested classes/.
      + /Non-static nested classes/ are called /inner classes/.

    - Java's /inner classes/ belong to its /outer class/.

    - Scala's /inner classes/ belong to *a specific /instance/ of its /outer class/.*
    ...
    - Illustration:
      #+begin_src scala
        ...
        object ClassInObject extends App {
          ...
          assert(ic1.isInstanceOf[oc1.InnerClass])
          assert(ic2.isInstanceOf[oc2.InnerClass])
          // assert(ic2.isInstanceOf[OuterClass.InnerClass])  /// Can't compile. No such a thing in Scala.
        }
      ...
      #+end_src
      + =from Jian=
        Both in Java and in Scala, we can't do ~new OuterClass.InnerClass()~!
        We also can't write the code like in this example in Java.
        In Java, we do
        #+begin_src java
          ...
          class OuterClass {
              ...
              public static void main(String[] args) {
                  ...
                  var ic1 = oc1.new InnerClass;
                  var ic2 = oc2.new InnerClass;
                  ...
                  assert ic1 instanceof OuterClass.InnerClass;
                  assert ic2 instanceof OuterClass.InnerClass;
                  // assert ic2 instanceof oc2.InnerClass;  /// Can't compile. No such a thing in Java
              }
          }
        #+end_src

    - _Classes inside an object_ and _objects inside a class_ are trivial
      #+begin_src scala
      ...
      #+end_src
      + =from Jian=
        /static nested classes/ is TRIVIAL both in Scala and Java -- they are just
        the classes in a specific scope. In Scala it is just the _classes inside
        an object_.
...
** TODO 5.10. Adding New Methods to Closed Classes (Extension Methods) - ???
* TODO 6. Objects - 171
...
** =DEPRECATED= 6.7. Putting Common Code in Package Objects - 182
*** DONE Problem - 182
...
** DONE 6.7. Creating Object Instances Without Using the ~new~ Keyword - 185
...
** TODO 6.9. Implement a Static Factory with ~apply~ - ???
*** DONE Problem - 189
...
** DONE 7.5. Using "Static" Members - 197
...
** DONE 9.1. Using Function Literals (Anonymous Functions) - 218
   CLOSED: [2020-05-08 Fri 03:06]
*** Problem - 218
*** Solution - 218
*** Discussion - 218

** DONE 9.2. Using Functions as Variables - 219
...
** DONE 9.3. Defining a Method That Accepts a Simple Function Parameter - 223
...
** TODO 9.4. Declaring More Complex Higher-Order Functions - 226
*** Problem - 226
*** Solution - 226
*** Discussion - 226
**** Passing in a function with other parameters - 227

** TODO 9.5. Using Partially Applied Functions - 234
*** Problem - 234
*** Solution - 234
*** Discussion - 235

** TODO 9.6. Creating a Function That Returns a Function - 236
*** Problem - 236
*** Solution - 236
*** Discussion - 237
**** Another example - 237
...
** TODO 9.7. Creating Partial Functions - ???
** TODO 9.8. How to Reify Traits as Objects - ???
** TODO 9.9. Real-World Example: Newton's Method - ???
*** Problem - 238
*** Solution - 238
...
** TODO 9.9. Real-World Example: Data and Functions - ???
** =DEPRECATED= 9.5. Using Closures - 229
*** Problem - 229
*** Solution - 229
*** Discussion - 230
**** A second example - 231
**** Using closures with other data types - 232
**** TODO A comparison to Java - 232
...
** TODO 10.6. How to Randomize a Sequence, of Get a Random Element - ???
** TODO 10.7. Understanding Mutable Variables with Immutable Collections - 265
** TODO 10.8. Make ~Vector~ Your "Go To" Immutable Sequence - 266
** TODO 10.9. Looping over a Collection with ~foreach~ - 270
** TODO 10.10. Looping over a Collection with a ~for~ Loop (TODO) - 272
...
** TODO 10.25. Sorting a Collection - ???
** TODO 10.26. Converting a Collection to a String with ~mkString~ - ???
** =DEPRECATED???=
** =DEPRECATED???= 10.8. Make ~ArrayBuffer~ Your "Go To" Mutable Sequence - 268
** =DEPRECATED???= 10.25. Populating a Collection with a Range - 309
** =DEPRECATED???= 10.26. Creating and Using Enumerations - 311
** =DEPRECATED???= 10.27. Tuples, for When You Just Need a Bag of Things - 312
...
** DONE 12.5. How to Process a CSV File - 384
   CLOSED: [2020-05-09 Sat 21:23]
*** Problem - 384
*** Solution - 384
    - =from Jian=
      This section gives some manually written solutions. Not libraries.

    - =from Jian=
      I only keep most realistic example:
      #+begin_src scala
        import scala.collection.mutable
        import scala.util.{io, Using}

        object CSVDemo3 extends App {
          val rows = new mutable.ArrayBuffer[Array[String]]

          Using.resource(io.Source.fromFile("/tmp/finance.csv")) { src =>
            for (ln <- src.getLines().drop(1)) {  // drop the header
...
*** Discussion - 387
    - You can try to search to find some open source csv processing projects.
...
** DONE 12.7. Using Serialization - 389
   CLOSED: [2020-05-09 Sat 22:36]
*** Problem - 389
    - Q :: How to _SERIALIZE a Scala class_ and _SAVE it as a file_, or _SEND it
           across a network_.

*** TODO Solution - 389
    The general approach is the *same* as Java,
    but the syntax to make a class serializable is *different*.
    - Before Scala 2.13, ~Serializable~ is a /trait/
      #+begin_src scala
        // For Java and .NET
        trait Serializable extends Any with java.io.Serializable
      #+end_src

    - Since Scala 2.13, ~Serializable~ is a /type alias/ of ~java.io.Serializable~.
    ...
    - TODO
      READ _Java “deep copy” technique that uses serialization_.
...
** DONE 12.8. Listing Files in a Directory - 391
...
** DONE 12.9. Listing Subdirectories Beneath a Directory - 392
   CLOSED: [2020-05-09 Sat 23:01]
*** Problem - 392
*** Solution - 392
    Similar like the last section (12.8), but this time list _direct_ subdirectories.
    #+begin_src scala
      // Assume that `dir` is a directory known to exist
      dir.listFiles
...
** DONE 12.11. Executing External Commands and Using STDOUT - 397
