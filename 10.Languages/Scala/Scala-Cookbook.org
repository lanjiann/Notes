...
- [[4. Classes and Properties - 99][4. Classes and Properties - 99]]
  ...
  - [[4.12. When to Use an Abstract Class - 129 - TODO =IMPORTANT=][4.12. When to Use an Abstract Class - 129 - TODO =IMPORTANT=]]
    - [[Problem - 129][Problem - 129]]
    - [[Solution - 129][Solution - 129]]
    - [[Discussion - 129][Discussion - 129]]
  - [[4.13. Defining Properties in an Abstract Base Class (or Trait) - 131 - TODO =RE-READ=][4.13. Defining Properties in an Abstract Base Class (or Trait) - 131 - TODO =RE-READ=]]
...
** DONE 4.3. Defining Auxiliary Constructors - 108
...
** DONE 4.4. Defining a Private Primary Constructor - 112
...
** DONE 4.7. Preventing Getter and Setter Methods from Being Generated - 119
   CLOSED: [2019-07-23 Tue 15:03]
*** DONE Problem - 119
    CLOSED: [2019-07-23 Tue 14:33]
    - We know
      + when we define a class field as a ~var~, a /getter/ and a /setter/ will be generated.
      + when we define a class field as a ~val~, a /getter/ will be generated.

    - Q :: How about if we don't want generate both /getter/ and /setter/?

*** DONE Solution - 119
    CLOSED: [2019-07-23 Tue 14:33]
    - A :: Use /modifier/ ~private~ or ~private[this]~.
    ...
    - Example:
      ...
      #+end_src
      + Disassemble it with =javap=, and you'll see this interface (=from Jian= here,
        interface in concept, not ~interface~ in Java):
        #+begin_src java
        ...
        #+end_src
        * TODO
          ??? Use what command line options of =javap= can show the /superclass/ part ???
          =from Jian=
          The super class part might not be the same for current version of
          Scala (2.11, 2.12, and 2.13).

*** DONE Discussion - 119
    CLOSED: [2019-07-23 Tue 15:03]
    ~private~ is for the class scope -- for example, in our example, _ANY_
    /instance/ of a ~Stock~ /class/ can access a /private field/ of _ANY
    OTHER_ ~Stock~ /instance/.
    #+begin_src scala
      class Stock {
      ...
      object Driver extends App {
        ...
        println(s2.isHigherThan(s1))
      }
    #+end_src

**** Object-private fields - 120
     Defining a field as ~private[this]~ takes this privacy a step further, and
     makes the /field/ /object-private/ -- it can only be accessed from the
     /object/ that _contins_ it.

** DONE 4.8. Assigning a Field to a Block or Function - 121
   CLOSED: [2019-07-23 Tue 15:27]
*** Problem - 121
*** Solution - 121
*** Discussion - 121
...
** DONE 4.12. When to Use an Abstract Class - 129 - TODO =IMPORTANT=
   CLOSED: [2019-07-23 Tue 16:00]
*** Problem - 129
    - The reason why we have this question:
        In most cases, _a Scala /trait/ is *more flexible* than an /abstract
      class/._
      There must be some exceptions, or why does the /abstract class/ exists?

    - Q :: When should I use an /abstract class/?

*** TODO Solution - 129
    - There are _TWO main reasons_ to use an /abstract class/ in Scala:
      + You want to create a /base class/ that requires /constructor arguments/.
        =from Jian= This reason will be disappear in Scala 3.

      + The code will be *called FROM Java code*.

    - Use an /abstract class/ whenever a base behavior *must* have /constructor parameters/.
      + =from Jian= TODO TODO TODO
        How can we define "MUST"? For instance, I'm not sure which is better in
        the cases below
        #+begin_src scala
    ...
    - TODO
      Regarding the SECOND reason, if you're writing code that needs to be
      accessed from Java, you'll find that /Scala traits/ WITH /implemented methods/
      can't be called from Java code.
      TODO
      TODO
      TODO
      If you run into this situation, see
      _Recipe 17.7, “Wrapping Traits with Implementations”_, for solutions to that problem.

*** TODO Discussion - 129
    Because a /class/ can extend *only one* /abstract class/,
    _WHEN_ you're trying to decide whether to use a /trait/ or /abstract class/,
    _ALWAYS_ use a /trait/, _UNLESS_ you have this /specific need/ (???) to have
    /constructor arguments/ in your base implementation.

** TODO 4.13. Defining Properties in an Abstract Base Class (or Trait) - 131 - TODO =RE-READ=
*** Problem - 131
*** Solution - 131
...
* DONE 7. Packaging and Imports - 191
...
** DONE 7.5. Using Static Imports - 197
...
** DONE 8.2. Using Abstract and Concrete Fields in Traits - 206
   CLOSED: [2019-07-25 Thu 14:50]
*** Problem - 206
*** Solution - 206
*** Discussion - 206
    - /Fields/ of a /trait/ can be declared as either ~var~ or ~val~.
      ...
      TODO =REDO= Recipie 4.13, "Defining Properties in an Abstract Base Class (or Trait)"
