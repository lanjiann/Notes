...
* 3. Basic types - 43
  - Rust's types serve several goals:
    ...
    + Concision:
      Rust manages all of this without requiring too much guidance from the
      programmer in the form of types written out in the code -- usually much
      less cluttered with types than the analogous C++ program would be.

  - Types can help Rust compiler to choose _GOOD machine-level_ /representations/
    for values your program operate on: /representations/ whose _performance_ you
    *can predict*, and which give you full access to the machine's capabilities.

  - As a /statically typed language/, Rust
    + *forces* the users to take more planning from the up front.
      The planning is materialized as _consistent_ data types in your program.

    + compiler checks the _consistency_ between data types, and catch many
      mistakes earlier, before running.
