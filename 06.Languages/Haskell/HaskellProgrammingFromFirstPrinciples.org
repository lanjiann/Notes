...
- [[10 Folding lists][10 Folding lists]]
  ...
  - [[10.4 [ =RE-READ= ] Fold right][10.4 [ =RE-READ= ] Fold right]]
...
- [[11 Algebraic datatypes][11 Algebraic datatypes]]
  ...
  - [[11.9 [ =Re-READ= ] ~newtype~][11.9 [ =Re-READ= ] ~newtype~]]
  ...
  - [[11.12 [ =Re-Note= ] Normal form][11.12 [ =Re-Note= ] Normal form]]
...
- [[14 [ =Reading= ] Testing  ------- Page 523 ~ 567][14 [ =Reading= ] Testing  ------- Page 523 ~ 567]]
...
- [[18 [ =Reading= ] ~Monad~][18 [ =Reading= ] ~Monad~]]
...
** DONE 9.8 Spines and nonstrict evaluation
   CLOSED: [2017-07-03 Mon 00:42]
...
** DONE 10.1 Folds
   ...
   - /catamorphisms/:
     ...
     Catamorphisms are a means of deconstructing data (but it doesn't say the
     structure won't be constructed again in this process -- constructions can
     also happen, though it is usually not).
...
** DONE 10.2 Bringing you into the fold
   ...
   - We can even recover the more concrete type because we can always make a
     type more concrete, but never more generic:
     #+BEGIN_SRC haskell
      -- Prelude> :{
...
** DONE 10.3 Recursive patterns
   ...
   Compare /fold/ and /map/:
   - /fold/ (applicable to all the foldable data structures):
     _fold folding-function fallback-val foldable-data_ 
     1. replace the constructor of the foldable data structure with the folding
        function;

     2. use passed in operator to reduce the foldable structure -- the last
        argument;

     3. get a single value, whose type depends on
   ...
   - /map/ (only applicable to lists):
     _map mapping-function list_
     1. Apply the mapping-function to each element of the list;

     2. Keep the /cons/ operator, and use it to combine the calculated values;

     3. Get a list with modified inside values.
        The type of those values depends on
        * The mapping function
        * the type of the original values in the passed in list

** DONE 10.4 [ =RE-READ= ] Fold right
   CLOSED: [2017-06-01 Thu 22:02]
   ~foldr~ is called _fold right_ because the fold is _right associative_.
   ...
   + The ~z~ is the /zero of our fold/. It provides a /fallback value/ for the
     empty list case and a second argument to begin our fold with.

     The zero is often the identity for whatever function we're folding with,
     such as 0 for ~(+)~ and 1 for ~(*)~.
     
*** DONE How ~foldr~ evaluates
        ...
        foldr (+) 0 [2, 3] =
          case [2, 3] of
            ...
            (1 : [2, 3]) ->
              (+) 1 (foldr (+) 0 [2, 3])
      #+END_SRC
      + Critically, ~(foldr (+) 0 [2, 3])~ should be expanded only because
        ~(\plus{})~ is /strict/ in _both_ of its arguments (NOT ONLY this is true for
        ~(\plus{})~, BUT ALSO this operation is _unconditional_), so it definitely
        forces the next iteration.

      + We could have a function which doesn't continually force the rest
        of the fold. If it were to stop on the first case here, then it
        would've returned the value ~1~. One such function is ~const~ which
    ...
    - A trick popularized by some helpful users in the Haskell IRC community (see
      ...
      A method used to show the process of ~foldr~ (=IMPORTANT=):
      #+BEGIN_SRC haskell
        xs = map show [1..5]
    ...
    - All folds recurse over the /spine/ _in the same direction_;
      =IMPORTANT=

      the _DIFFERENCE_ between /left folds/ and /right folds/ is in the
      association of the folding function and, thus, which direction the folding
    ...
    - Look at part of the definition of ~foldr~:
      ...
      often._ One of the CONSEQUENCES of this is that ~foldr~ can _avoid_
      evaluating
    ...
    - ~take~ is non-strict like everything else you've seen so far.
      ...
      + =TODO=   page 357 - _START HERE_

      =From Jian=
      Speak this in another way: ~take 2~ doesn't happen after when
      ~take 4 ([1, 2]++undefined)~ is done ----
      the left ~take~ takes the first element when the right ~take~ takes the
      first element, and the left ~take~ takes the second element when the right
      ~take~ takes the the second element. Now the left ~take~ has finished its
      work, then the right ~take~ would also stop doing it planed to do but no
      longer need to do due to /laziness/.
...
** DONE 11.12 [ =Re-Note= ] Normal form
...
*** DONE Exercises: The Quad
    ...
    6. 65536
       (4 ^ 4) ^ 2
** DONE 11.15 Higher-kinded datatypes
...
* TODO 14 [ =Reading= ] Testing  ------- Page 523 ~ 567
** DONE 14.1 Testing
...
** DONE 14.6 Kicking around ~QuickCheck~
...
** DONE 17.5 ~Applicative~ in use
   CLOSED: [2017-07-02 Sun 17:47]
*** DONE List ~Applicative~
...
**** DONE What's the List applicative do?
     CLOSED: [2017-07-01 Sat 23:08]
     - Equivalent forms:
       #+BEGIN_SRC haskell
         (,) <$> [1, 2] <*> [3, 4]
...
**** DONE Exercise: Identity Instance
...
**** DONE Exercise: Constant Instance
     CLOSED: [2017-07-02 Sun 02:39]
     #+BEGIN_SRC haskell
...
**** DONE Using the ~Maybe~ Applicative
     CLOSED: [2017-07-02 Sun 03:08]
     #+BEGIN_SRC haskell
...
**** DONE Breaking down the example we just saw
     CLOSED: [2017-07-02 Sun 03:09]
**** DONE ~Maybe~ Functor and the Name constructor
...
**** DONE ~Maybe~ Applicative and Person
     CLOSED: [2017-07-02 Sun 17:36]

**** DONE Before we moooove on
