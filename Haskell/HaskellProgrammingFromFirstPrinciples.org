...
** DONE 10.4 [ =RE-READ= ] Fold right
   ...
   - _Fold right_ means the fold is _right associative_.
     ...
     #+END_SRC
     Here ~z~ is the _zero_ (or nothing, or base) of our fold, and
     it provides
...
*** DONE How ~foldr~ evaluates
    ...
    - A trick popularized by some helpful users in the Haskell IRC community:
      ...
      footnote: Idea borrowed from Cale Gibbard from the haskell Freenode IRC
      channel and on the Haskell.org wiki https://wiki.haskell.org/Fold#Examples
